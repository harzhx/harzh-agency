          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16 animate-in fade-in duration-300">
            {filteredItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="rounded-3xl border border-white/[0.08] hover:border-indigo-500/40 bg-[#090b14]/90 backdrop-blur-2xl p-2.5 transition-all duration-300 flex flex-col justify-between shadow-2xl group ring-1 ring-white/5"
                >
                  ...
                  {/* Compact Telemetry Row (No Clutter) */}
                  <div className="px-2 pt-3 pb-1 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-[11px] text-white/50 font-mono mt-0.5">
                        <span className="text-white/80 font-medium">{item.creator}</span>
                        <span>•</span>
                        <span className="text-emerald-400 font-bold">{item.views} Views</span>
                        <span>•</span>
                        <span className="hidden sm:inline text-white/40">{item.tags[0]}</span>
                      </div>
                    </div>

                    <button
                      onClick={onOpenBooking}
                      className="shrink-0 px-3 py-1.5 rounded-xl bg-white/[0.04] hover:bg-white text-white hover:text-black border border-white/[0.08] hover:border-white text-[11px] font-mono font-semibold transition-all flex items-center gap-1 shadow-md"
                    >
                      <span>Apply</span>
            ...
            {filteredItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="relative aspect-[9/14] rounded-2xl overflow-hidden bg-black ring-1 ring-white/10">
                    {isPlaying ? (
                      <video
                        src={item.videoPlaceholderUrl}
                        poster={item.thumbnailUrl}
                  ...
                  {/* Compact Telemetry Row for Shorts */}
                  <div className="px-2 pt-2.5 pb-1 flex items-center justify-between gap-2">
                    <span className="text-white/60 font-mono text-[11px] truncate">
                      {item.keyEdits[0]}
                    </span>
                    <button
                      onClick={onOpenBooking}
                      className="shrink-0 px-2.5 py-1 rounded-lg bg-white/[0.04] hover:bg-white text-white hover:text-black border border-white/[0.08] text-[10px] font-mono transition-all flex items-center gap-1"
                    >
                      <span>Apply</span>
                      <ArrowUpRight className="w-3 h-3" />
                    </button>
