    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <motion.div
          ...
          className="hidden sm:grid sm:grid-cols-4 gap-2.5 md:gap-3.5 lg:gap-4 max-w-5xl mx-auto mb-8 sm:mb-12"
        >
          {AGENCY_STATS.map((stat, index) => (
            <div
              key={index}
              className="p-3.5 sm:p-4 md:p-5 lg:p-6 rounded-2xl border border-white/[0.08] bg-white/[0.02] text-center backdrop-blur-md hover:border-white/20 transition-all shadow-lg flex flex-col justify-center min-h-[96px] sm:min-h-[108px] md:min-h-[116px] lg:min-h-[120px]"
            >
              <div className="text-2xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold font-sans tracking-tight text-white mb-1 md:mb-1.5 whitespace-nowrap">
                {stat.value}
              </div>
              <div className="text-[10px] sm:text-[10.5px] md:text-xs font-semibold uppercase tracking-wider text-white/70 leading-relaxed font-sans">
                {stat.label}
              </div>
            </div>
          ))}
